        ...
        try {
          ...
          await exec.exec('sudo', [
            ...
            `set -x
             ...
             i=0
             while ! /Applications/Docker.app/Contents/Resources/bin/docker system info; do
               (( i++ == 0 )) && printf %s '-- Waiting for Docker to finish starting up...' || printf '.'
               command -v docker || echo 'test docker command loop: not found'
               sleep 1
               # wait 10s
               if [ $i -gt 10 ]; then
                 sudo /Applications/Docker.app/Contents/MacOS/com.docker.diagnose check;
                 uname -a;
                 system_profiler SPHardwareDataType;
                 echo "::error::-- Wait docker start $i s too long, exit"; 
                 exit 1; 
             fi
             done
          ...
          break;
            ...
            tries++;
            // Kill the docker process if it's running
            await exec.exec('sudo', ['pkill', '-9', 'Docker']);
        }
    }
    core.endGroup();
